...
#     "pchlenski/gpt2-transcoders", f"{version}_cfg.json"
# )
layer = 0
...
# pprint.pprint(cfg)
# self = cls(cfg=cfg)
info = utils.download_file_from_hf(repo, filename, force_is_torch=True)
...
# %%
trans = SparseTranscoder.load_from_pretrained(file_path).train(False)
...
# %%
stuff = json.load(open(file_path, "r"))
...
_, cache = model.run_with_cache(
    model.to_tokens("14. Colorado 15. Missouri 16. California")
)

# %%
layer = 0

t_in = cache["normalized", layer, "ln2"]
pred = trans(t_in)[0]
(cache["mlp_out", layer] - pred).norm(dim=-1).mean()

# %%
a = 32

real = einops.rearrange(cache["mlp_out", layer][0, -1], "(a b) -> a b", a=a)
pred_r = einops.rearrange(pred[0, -1], "(a b) -> a b", a=a)
...
# %%
print_model_memory_usage(trans)
