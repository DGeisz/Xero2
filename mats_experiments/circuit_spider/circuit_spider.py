...
class ActiveFeatures:
    ...
    def get_vectors_before_comp(self, kind: str, layer: int):
        ...
        for i in range(layer):
            max_index += (
                self.keys[i]["mlp"] + self.keys[i]["attn"] + 4
            )  # include error and bias terms

        if kind == "mlp":
            max_index += self.keys[layer]["attn"] + 2
    ...
    def get_reconstructed_attn_out(self, layer: int):
        ...
        num_comps = self.keys[layer]["attn"] + 2
    ...
    def get_reconstructed_mlp_out(self, layer: int):
        ...
        num_comps = self.keys[layer]["mlp"] + 2

        return self.vectors[start_i : start_i + num_comps].sum(dim=0)
    ...
    def reshape_activations_for_visualization(
        ...
        for i, key in enumerate(self.keys):
            ...
            if start_i + key["attn"] + 2 > a_len:
                break

            visualization[ii, :2] = activations[start_i : start_i + 2]
            start_i += 2
            ...
            if start_i + key["mlp"] + 2 > a_len:
                break

            visualization[ii + 2, :2] = activations[start_i : start_i + 2]
            start_i += 2
...
class PromptSpiderWeb:
    def __init__(self, spider, prompt):
        ...
        self.logits, self.cache = self.spider.model.run_with_cache(
    ...
    def test_compare_sae_out(self, layer, seq_index):
        ...
        test_sae_out = self.get_active_features(seq_index).get_sae_out_reconstruction(
            layer
        )

        _, z_recon, _, _, _ = self.z_saes[layer](layer_z)

        print("recon sahpe", z_recon.shape, test_sae_out.shape)
        ...
        return (
            torch.allclose(z_recon, test_sae_out, atol=1e-5),
    ...
    def test_compare_max_attn_features(self, layer: int, seq_index):
        ...
        return (
            torch.allclose(z_residual_vectors, max_features),
            (z_residual_vectors - max_features).norm(),
        )
    ...
    def test_compare_attn_out(self, layer: int, seq_index: int):
        ...
        active_features = self.get_active_features(seq_index)
        feature_recon = (
            active_features.get_reconstructed_attn_out(layer) + self.model.b_O[layer]
        )

        attn_out = self.cache["attn_out", layer][0, seq_index]

        return (
            torch.allclose(feature_recon, attn_out, atol=1e-5),
    ...
    def test_compare_mlp_out(self, layer: int, seq_index: int):
        ...
        return (
            torch.allclose(feature_recon, mlp_out),
            (feature_recon - mlp_out).norm(),
        )

    def get_active_features(self, seq_index: int) -> ActiveFeatures:
        seq_index = self.process_seq_index(seq_index)
        ...
        for layer in trange(self.model.cfg.n_layers):
            ...
            print("z_acts shape:", z_acts.shape)
