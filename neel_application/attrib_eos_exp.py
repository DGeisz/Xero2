...
def get_normalizing_function_for_tokens(
    clean_tokens=None, corrupted_tokens=None, clean_cache=None, corrupted_cache=None
):
    ...
    clean_activation = get_linear_feature_activation_from_cache(
        clean_cache, averaged=True
    )
    corrupted_activation = get_linear_feature_activation_from_cache(
        corrupted_cache, averaged=True
    )
...
def fetch_layer_9_z_activation(activation, hook, store):
...
def run_activation_patching(
    clean_tokens, corrupted_tokens, hook_name, component_title, return_patch=False
):
    _, clean_cache = model.run_with_cache(clean_tokens)

    normalize = get_normalizing_function_for_tokens(
        clean_cache=clean_cache, corrupted_tokens=corrupted_tokens
    )
    ...
    for layer in range(model.cfg.n_layers):
        for position in range(clean_tokens.shape[1]):
            ...
            feature_activation = get_linear_feature_activation(
                layer_9_z_store[0], averaged=True
            )
...
def run_activation_corruption(
    clean_tokens, corrupted_tokens, hook_name, component_title, return_patch=False
):
    _, clean_cache = model.run_with_cache(clean_tokens)
    _, corrupted_cache = model.run_with_cache(corrupted_tokens)

    base_activation = get_linear_feature_activation_from_cache(
        clean_cache, averaged=True
    )
    ...
    for layer in range(model.cfg.n_layers):
        for position in range(clean_tokens.shape[1]):
            ...
            hook_fn = partial(
                patch_activation, pos=position, clean_cache=corrupted_cache
            )
            fetch_fn = partial(fetch_layer_9_z_activation, store=layer_9_z_store)
            ...
            patched_diff[layer, position] = normalize_corruption_result(
                feature_activation, base_activation
            )
...
def run_activation_zero_ablation(
    clean_tokens, hook_name, component_title, return_patch=False
):
    _, clean_cache = model.run_with_cache(clean_tokens)
...
run_activation_patching_on_residual_stream = partial(
    run_activation_patching, hook_name="resid_pre", component_title="Residual Stream"
)
run_activation_patching_on_attn_output = partial(
    run_activation_patching, hook_name="attn_out", component_title="Attention Output"
)
run_activation_patching_on_mlp_output = partial(
    run_activation_patching, hook_name="mlp_out", component_title="MLP Output"
)

run_activation_corruption_on_residual_stream = partial(
    run_activation_corruption, hook_name="resid_pre", component_title="Residual Stream"
)
run_activation_corruption_on_attn_output = partial(
    run_activation_corruption, hook_name="attn_out", component_title="Attention Output"
)
run_activation_corruption_on_mlp_output = partial(
    run_activation_corruption, hook_name="mlp_out", component_title="MLP Output"
)


run_zero_ablation_on_residual_stream = partial(
    run_activation_zero_ablation,
    hook_name="resid_pre",
    component_title="Residual Stream",
)
run_zero_ablation_on_attn_output = partial(
    run_activation_zero_ablation,
    hook_name="attn_out",
    component_title="Attention Output",
)
run_zero_ablation_on_mlp_output = partial(
    run_activation_zero_ablation, hook_name="mlp_out", component_title="MLP Output"
)
...
def bos_ablate_head_vector(
    head_vector: Float[torch.Tensor, "batch pos head_index d_head"],
    hook,
    head_index,
):
    bos_activations = einops.repeat(
        head_vector[:, 0, :, :],
        "N n_heads d_heads -> N pos n_heads d_heads",
        pos=head_vector.shape[1],
    )
    head_vector[:, :, head_index, :] = bos_activations[:, :, head_index, :]
...
def run_attention_activation_patching(
    clean_tokens,
    corrupted_tokens,
    patching_function,
    hook_name,
    ...
    for layer in range(model.cfg.n_layers):
        for head_index in range(model.cfg.n_heads):
            ...
            hook_fn = partial(
                patching_function, head_index=head_index, clean_cache=clean_cache
            )
            fetch_fn = partial(fetch_layer_9_z_activation, store=layer_9_z_store)
            ...
            patched_head_diff[layer, head_index] = normalize(feature_activation)
...
def run_attention_activation_corruption(
    clean_tokens,
    corrupted_tokens,
    patching_function,
    hook_name,
    ...
    for layer in range(model.cfg.n_layers):
        for head_index in range(model.cfg.n_heads):
            ...
            hook_fn = partial(
                patching_function, head_index=head_index, clean_cache=corrupted_cache
            )
            fetch_fn = partial(fetch_layer_9_z_activation, store=layer_9_z_store)
            ...
            patched_head_diff[layer, head_index] = normalize_corruption_result(
                feature_activation, base_value
            )
...
def run_attention_ablation(
    clean_tokens, patching_function, hook_name, component_title, return_patch=False
):
    _, clean_cache = model.run_with_cache(clean_tokens)
    ...
    for layer in trange(model.cfg.n_layers):
        for head_index in range(model.cfg.n_heads):
            ...
            head_diff[layer, head_index] = normalize_corruption_result(
                feature_activation, base_value
            )
...
run_activation_patching_on_z_output = partial(
    run_attention_activation_patching,
    patching_function=patch_head_vector,
    hook_name="z",
    component_title="Z Output",
)
run_activation_patching_on_values = partial(
    run_attention_activation_patching,
    patching_function=patch_head_vector,
    hook_name="v",
    component_title="Attention Values",
)
run_activation_patching_on_attn_pattern = partial(
    run_attention_activation_patching,
    patching_function=patch_head_pattern,
    hook_name="attn",
    component_title="Attention Pattern",
)

run_activation_corruption_on_z_output = partial(
    run_attention_activation_corruption,
    patching_function=patch_head_vector,
    hook_name="z",
    component_title="Z Output",
)
run_activation_corruption_on_values = partial(
    run_attention_activation_corruption,
    patching_function=patch_head_vector,
    hook_name="v",
    component_title="Attention Values",
)
run_activation_corruption_on_attn_pattern = partial(
    run_attention_activation_corruption,
    patching_function=patch_head_pattern,
    hook_name="attn",
    component_title="Attention Pattern",
...
run_zero_ablation_on_attn_head = partial(
    run_attention_ablation,
    patching_function=zero_ablate_head_pattern,
    hook_name="attn",
    component_title="Attention Pattern",
)

run_bos_ablation_on_attn_head = partial(
    run_attention_ablation,
    patching_function=bos_ablate_head_vector,
    hook_name="z",
    component_title="Z (BOS)",
)


# %%
def corrupt_activation(
    original_comp: Float[torch.Tensor, "batch pos head_index d_head"],
    hook,
    pos,
    head_index,
    replace_cache,
):
    original_comp[:, pos, head_index, :] = replace_cache[hook.name][
        :, pos, head_index, :
    ]
...
def run_attention_activation_corruption_on_head(
    clean_tokens, corrupted_tokens, heads: List[Tuple[int, int]], return_patch=False
):
    _, clean_cache = model.run_with_cache(clean_tokens)
    _, corrupted_cache = model.run_with_cache(corrupted_tokens)
    ...
    for i, (layer, head_index) in enumerate(heads):
        for position in range(clean_tokens.shape[1]):
            ...
            head_diff[i, position] = normalize_corruption_result(
                feature_activation, base_value
            )
...
def run_attention_activation_patching_on_head(
    clean_tokens, corrupted_tokens, heads: List[Tuple[int, int]], return_patch=False
):
    _, clean_cache = model.run_with_cache(clean_tokens)
...
def run_isolated_circuit(
    ...
    model.run_with_hooks(
        ...
        fwd_hooks=[
            (
                lambda name: name.startswith("blocks") and name.endswith("hook_z"),
                hook_fn,
            ),
            (utils.get_act_name("z", 9), fetch_fn),
        ],
    ...
    isolated_activation = get_linear_feature_activation(
        layer_9_z_store[0], averaged=False
    )
...
pattern_output_scatter_plot = partial(
    create_head_patching_scatter_plot, x_name="Attention", y_name="Output"
)
values_output_scatter_plot = partial(
    create_head_patching_scatter_plot, x_name="Value", y_name="Output"
)
...
def print_prompt_stats(prompts, model=model):
    ...
    print(
        ...
        [
            float(f"{n:.2f}")
            for n in get_linear_feature_activation_from_cache(
                cache, averaged=False
            ).tolist()
        ],
    )
...
def run_full_analysis(clean_tokens, corrupted_tokens):
    ...
    run_activation_patching_on_z_output(
        clean_tokens, corrupted_tokens, return_patch=True
    )
    run_activation_corruption_on_z_output(
        clean_tokens, corrupted_tokens, return_patch=True
    )

    run_activation_patching_on_values(clean_tokens, corrupted_tokens, return_patch=True)
    run_activation_corruption_on_values(
        clean_tokens, corrupted_tokens, return_patch=True
    )

    run_activation_patching_on_attn_pattern(
        clean_tokens, corrupted_tokens, return_patch=True
    )
    run_activation_corruption_on_attn_pattern(
        clean_tokens, corrupted_tokens, return_patch=True
    )
...
random_prefix_corrupted_prompts = [
    successor_prompt_format.format("and", "the", triple[2]) for triple in state_triples
]
random_prefix_corrupted_tokens = model.to_tokens(
    random_prefix_corrupted_prompts, prepend_bos=True
)
...
name_corrupted_prompts = [
    successor_prompt_format.format(pair[0][0], pair[0][1], pair[1])
    for pair in zip(state_triples, people_names)
]
...
number_corrupted_prompts = [
    corrupted_number_prompt.format(*triple) for triple in state_triples
]
...
prefix_number_corrupted_prompts = [
    prefix_number_corrupted_prompt_template.format(*triple) for triple in state_triples
]
prefix_number_corrupted_tokens = model.to_tokens(
    prefix_number_corrupted_prompts, prepend_bos=True
)
...
long_distance_prompts = [
    long_distance_prompt.format(*pair) for pair in people_names_prefix
]
...
# %%
long_distance_prefix_name_corrupted_prompt = (
    "14. and 15. then we have much later 16. {}"
)

long_distance_prefix_name_corrupted_prompts = [
    long_distance_prefix_name_corrupted_prompt.format(pair[1])
    for pair in people_names_prefix
]
long_distance_prefix_name_corrupted_tokens = model.to_tokens(
    long_distance_prefix_name_corrupted_prompts, prepend_bos=True
)

print_prompt_stats(long_distance_prefix_name_corrupted_prompts)

# %%
long_distance_second_number_corrupted_prompt = (
    "14. {} beta. then we have much later 16. {}"
)

long_distance_second_number_corrupted_prompts = [
    long_distance_second_number_corrupted_prompt.format(*pair)
    for pair in people_names_prefix
]
long_distance_second_number_corrupted_tokens = model.to_tokens(
    long_distance_second_number_corrupted_prompts, prepend_bos=True
)

print_prompt_stats(long_distance_second_number_corrupted_prompts)
# %%
long_distance_first_number_corrupted_prompt = (
    "alpha and {} 15. then we have much later 16. {}"
)

long_distance_first_number_corrupted_prompts = [
    long_distance_first_number_corrupted_prompt.format(*pair)
    for pair in people_names_prefix
]
long_distance_first_number_corrupted_tokens = model.to_tokens(
    long_distance_first_number_corrupted_prompts, prepend_bos=True
)
...
long_distance_last_number_corrupted_prompts = [
    long_distance_last_number_corrupted_prompt.format(*pair)
    for pair in people_names_prefix
]
long_distance_last_number_corrupted_tokens = model.to_tokens(
    long_distance_last_number_corrupted_prompts, prepend_bos=True
)
...
# %%
run_full_analysis(long_distance_tokens, long_distance_last_number_corrupted_tokens)
...
# %%
z_ab = run_zero_ablation_on_attn_head(clean_tokens, return_patch=True)


# %%
bos_ab = run_bos_ablation_on_attn_head(clean_tokens, return_patch=True)
...
def get_cache_forward_backward(model, tokens):
    model.reset_hooks()
    cache = {}
    ...
    model.add_hook(utils.get_act_name("z", 9), fetch_fn, "fwd")
    ...
    value = get_linear_feature_activation(layer_9_z_store[0])
    value.backward()
    model.reset_hooks()
...
# %%
value, cache, grad_cache = get_cache_forward_backward(model, clean_tokens)


# %%
def create_attn_attr(clean_cache, grad_cache):
    seq_len = clean_cache["z", 0].shape[1]
    ...
    bos_stack = torch.stack(
        [
            einops.repeat(
                clean_cache["z", layer][:, 0, ...], "B ... -> B seq ... ", seq=seq_len
            )
    ...
    z_stack = torch.stack(
        [clean_cache["z", layer] for layer in range(last_layer)], dim=0
    )
    z_grad_stack = torch.stack(
        [grad_cache["z", layer] for layer in range(last_layer)], dim=0
    )
    ...
    attention_attr = z_stack * z_grad_stack
    attention_attr = einops.reduce(
        attention_attr, "layer batch seq head_index d_head -> layer head_index", "sum"
    )

    return bos_attention_attr, attention_attr


# %%
bos_attn_attr, attn_attr = create_attn_attr(cache, grad_cache)


# %%
imshow(attn_attr), imshow(bos_attn_attr)

# %%
clean_cache["z", 0][:, :1, ...].shape
...
model.blocks[0].mlp.W_in.grad.shape
...
# %%
clean_cache["pattern", 4].shape
grad_cache["pattern", 4].shape

# %%
layer = 1
head = 10

movement = einops.reduce(
    clean_cache["pattern", layer] * grad_cache["pattern", layer],
    "b head x y -> head x y",
    "sum",
)
movement = grad_cache["pattern", 4].sum(dim=0)

cv.attention.attention_heads(
    attention=movement[head].unsqueeze(0),
    tokens=model.to_str_tokens(clean_tokens[0]),
    attention_head_names=["L$H$"],
)

# %%
plot = cv.attention.attention_heads(
    attention=movement[4].unsqueeze(0),
    tokens=model.to_str_tokens(clean_tokens[0]),
    attention_head_names=["L$H$"],
).show_code()

# Display the title
title_html = f"<h2>Yup</h2><br/>"

# Return the visualisation as raw code
HTML(f"<div style='max-width: {str(600)}px;'>{title_html + plot}</div>")
...
HTML(
    visualize_attention_patterns(
        [
            *list(range(layer * 12, (layer + 1) * 12)),
            # head_index(0, 1),
            # head_index(1, 5),
            # head_index(1, 3),
            # head_index(1, 10),
